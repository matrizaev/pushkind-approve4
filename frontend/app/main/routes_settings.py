...
def show_settings():
    ...
    forms["edit_category"].cashflow.choices = [
        (c["name"], c["name"]) for c in cashflows
    ]
    forms["edit_category"].budget_holder.choices = [
        (b["name"], b["name"]) for b in budget_holders
    ]
    forms["edit_category"].responsible.choices = [
        (u["email"], u["name"]) for u in users if u["role"]["name"] == "purchaser"
    ]
    ...
    if current_user.role.name == "admin":
        ...
        forms["user"].role.choices = [(r["name"], r["pretty"]) for r in roles]
    else:
        forms["user"] = UserSettingsForm()

    if current_user.role.name in ("admin", "purchaser", "validator"):
        forms["user"].about_user.categories.choices = [c["name"] for c in categories]
        forms["user"].about_user.projects.choices = [p["name"] for p in projects]
    else:
...
def edit_user():
    ...
    if current_user.role.name == "admin":
        ...
        user_form.role.choices = [(r["name"], r["pretty"]) for r in roles]
    else:
        user_form = UserSettingsForm()

    if current_user.role.name in ("admin", "purchaser", "validator"):
        user_form.about_user.categories.choices = [c["name"] for c in categories]
        user_form.about_user.projects.choices = [p["name"] for p in projects]
    else:
        ...
        if user_form.validate_on_submit():
            if current_user.role.name == "admin":
                user = {
                    ...
                    "birthday": user_form.birthday.data.isoformat()
                    if user_form.birthday.data
                    else None,
                }
