...
class EditHubForm(FlaskForm):
    hub_id = IntegerField(
        "ID хаба", validators=[DataRequired(message="ID хаба - обязательное поле.")]
    )
    hub_name = StringField(
        "Название",
        validators=[
...
class AddVendorForm(FlaskForm):
    ...
    password = PasswordField(
        "Пароль",
        validators=[DataRequired(message="Пароль поставщика - обязательное поле.")],
    )
    submit = SubmitField("Создать")
...
class AppSettingsForm(FlaskForm):
    ...
    alert = TextAreaField(
        "Предупреждение",
        validators=[Length(max=512, message="Слишком длинное название.")],
    )
    submit = SubmitField("Сохранить")
...
class EditCategoryForm(FlaskForm):
    ...
    budget_holder = SelectField(
        "ФДБ", validators=[DataRequired(message="ФДБ - обязательное поле.")], coerce=str
    )
    income = SelectField(
        "БДР", validators=[DataRequired(message="БДР - обязательное поле.")], coerce=str
    )
