...
from app.main.utils import products_json_to_excel
...
def upload_products():
    ...
    if form.validate_on_submit():
        ...
        response = ProductApi.post_entity(products=data, vendor_id=vendor_id)
    ...
    if form.validate_on_submit():
        ...
        with ZipFile(form.images.data, "r") as zip_file:
            for zip_info in zip_file.infolist():
                if (
                    zip_info.is_dir()
                    or zip_info.file_size > current_app.config["MAX_FILE_SIZE"]
                ):
                    continue
...
def download_products():
    ...
    products = ProductApi.get_entities(vendor_id=vendor_id) or []
    buffer = products_json_to_excel(products)
    return send_file(
        buffer,
        as_attachment=True,
...
def add_hub():
    ...
    if form.validate_on_submit():
        ...
        errors = form.hub_name.errors + form.email.errors
        for error in errors:
            flash(error)
    return redirect(url_for("main.show_vendors"))
...
def add_vendor():
    ...
    if form.validate_on_submit():
        ...
        response = VendorApi.post_entity(name=vendor_name, email=vendor_email)
        if response is not None:
            ...
            if response is None:
                ...
                response = UserApi.put_entity(entity_id=response["id"])
...
def edit_hub():
    ...
    if form.validate_on_submit():
        ...
        data = {"name": form.hub_name.data.strip(), "enabled": form.enabled.data}
        ...
        errors = form.hub_id.errors + form.hub_name.errors + form.enabled.errors
        for error in errors:
            flash(error)
    return redirect(url_for("main.show_vendors"))
...
def edit_vendor():
    ...
    if form.validate_on_submit():
        ...
        data = {"name": form.vendor_name.data.strip(), "enabled": form.enabled.data}
        ...
        errors = form.vendor_id.errors + form.vendor_name.errors + form.enabled.errors
        for error in errors:
            flash(error)
    return redirect(url_for("main.show_vendors"))
