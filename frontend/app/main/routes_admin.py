...
def edit_category():
    ...
    if form.validate_on_submit():
        ...
        response = CategoryApi.put_entity(
            ...
            responsible=first(
                filter(lambda x: x["email"] == form.responsible.data, responsibles)
            ),
...
def add_income():
    form = IncomeForm()
    if form.validate_on_submit():
        response = IncomeApi.post_entity(name=form.income_name.data.strip())
        if response is not None:
            flash("БДР добавлен.")
        else:
...
def add_cashflow():
    form = CashflowForm()
    if form.validate_on_submit():
        response = CashflowApi.post_entity(name=form.cashflow_name.data.strip())
        if response is not None:
            flash("БДДС добавлен.")
        else:
...
def edit_income(income_id):
    ...
    if form.validate_on_submit():
        response = IncomeApi.put_entity(
            entity_id=income_id, name=form.income_name.data.strip()
        )
        if response is not None:
            flash("БДР изменён.")
        else:
...
def edit_budget_holder(budget_holder_id):
    ...
    if form.validate_on_submit():
        response = BudgetHolderApi.put_entity(
            entity_id=budget_holder_id, name=form.budget_holder_name.data.strip()
        )
        if response is not None:
            flash("ФДБ изменён.")
        else:
...
def edit_cashflow(cashflow_id):
    ...
    if form.validate_on_submit():
        response = CashflowApi.put_entity(
            entity_id=cashflow_id, name=form.cashflow_name.data.strip()
        )
        if response is not None:
            flash("БДДС изменён.")
        else:
