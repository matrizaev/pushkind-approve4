...
def get_orders():
    ...
    orders = Order.query.filter_by(hub_id=current_user["hub_id"]).filter_by(**args)
    if ids:
        orders = orders.filter(Order.id.in_(ids))
    ...
    if timestamp is not None:
        orders = orders.filter(
            Order.timestamp > datetime.fromtimestamp(float(timestamp), tz=timezone.utc)
        )
    if current_user["role"]["name"] == "initiative":
        ...
        orders = orders.join(OrderPurchaser).filter(
            OrderPurchaser.email == current_user["email"]
        )
    elif current_user["role"]["name"] == "validator":
        orders = orders.join(OrderValidator).filter(
            OrderValidator.email == current_user["email"]
        )
    elif current_user["role"]["name"] == "vendor":
        orders = orders.join(OrderVendor).filter(
            OrderVendor.email == current_user["email"]
        )
    orders = orders.order_by(Order.timestamp.desc()).all()
    return jsonify([o.to_dict("id" in args or ids) for o in orders]), 200
