...
def callback_hub_changed(ch, method, properties, body):
    ...
    with app.app_context():
        ...
        if removed:
            Tender.query.filter_by(hub_id=hub_id).delete()
        db.session.commit()
...
def main():
    ...
    channel.queue_declare(queue='hub_changed_project')
    channel.queue_bind(exchange='hub_changed', queue='hub_changed_project')

    channel.basic_consume(queue='hub_changed_project', on_message_callback=callback_hub_changed)
