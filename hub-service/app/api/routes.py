...
def get_hubs():
    ...
    hubs = Vendor.query.filter_by(hub_id=None).filter_by(**request.args)
...
def get_vendors():
    ...
    vendors = Vendor.query.filter_by(hub_id=current_user["hub_id"]).filter_by(**args)
...
def get_categories():
    ...
    if hub is None:
        return error_response(404, "Хаб не существует.")
    categories = (
        Category.query.filter_by(hub_id=current_user["hub_id"]).filter_by(**args).all()
    )
...
def get_app_settings():
    ...
    if hub is None:
        return error_response(404, "Хаб не существует.")
    app_settings = AppSettings.query.filter_by(hub_id=current_user["hub_id"]).first()
...
def post_category():
    ...
    category = Category.query.filter(
        func.lower(Category.name) == func.lower(data["name"]), Category.hub_id == hub.id
    ).first()
...
@token_auth.login_required(role=["admin", "vendor", "purchaser"])
def post_product():
    current_user = token_auth.current_user()
    hub = Vendor.query.filter_by(id=current_user["hub_id"], hub_id=None).first()
    if hub is None:
    ...
    if (
        current_user["role"]["name"] == "vendor"
        and vendor.email != current_user["email"]
    ):
    ...
    if data.get("products") is not None:
        ...
        category = Category.query.filter_by(
            hub_id=current_user["hub_id"], name=data["category"]
        ).first()
        if category is None:
